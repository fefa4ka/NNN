...
static void                 network_train(neural_network *network, matrix *signal, matrix *target, float learning_rate);
...
__network_seed_layer(neural_network *network, neural_layer *layer) {
    ...
    for(size_t position = 0; position < layer->dimension; position++) {
        neural_cell cell = {
            ...
            .axon = malloc(sizeof(struct NeuralCell *)),
            .synapse = malloc(sizeof(struct NeuralCell *)),
            .impulse = calloc(1, sizeof(enum bool)),
            .error = calloc(1, sizeof(enum bool))
        };
    ...
    while(layer_size > position) {
        ...
        if(derivative) {
            ...
            error[position] = cell->body.nucleus.error.derivative(cell->body.axon,
                                                                  neuron_target);
            vector *err = cell->body.nucleus.error.of(cell->body.axon,
                                                      neuron_target);
            log_info("[ Error: %.8f ] ~> ", Vector.sum.all(err) / err->size);
...
static
void
network_train(neural_network *network, matrix *signal, matrix *target, float learning_rate) {
    int epoch = 0;
...
neuron_error_impulse(neural_cell *source, neural_cell *destination) {
    ...
    while(destination->axon[index]) {
        if(destination->axon[index] == source) {
            destination->error[index] = true;
            impulse = true;
        }
        
        if(destination->error[index] == false) {
            fire = false;
        }
    ...
    if(fire) {
        ...
        destination->error = calloc(index, sizeof(enum bool));
